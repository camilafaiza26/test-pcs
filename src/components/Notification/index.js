...
import {IconAccepted, IconArrowRight, IconRejected} from '../../assets';
...
const Notification = ({title, total, submission, time, status, isViewed}) => {
  ...
  const statusToIcon = {
    approved: IconAccepted,
    paid:IconAccepted,
    rejected: IconRejected,
    reviewed: IconArrowRight,
    processed: IconArrowRight
  };
    ...
    if (status === 'rejected' && title === 'reimbursement') {
      return (
        <CustomText className="text-xs">
          has been
          <CustomText style={styles.bold}> {status}</CustomText>, please click
          for details.
        </CustomText>
      );
      ...
      return (
        <CustomText className="text-xs">
          {' '}
          will be
          <CustomText style={styles.bold}> {status}</CustomText> according to
        ...
        <CustomText className="text-xs">
          {status && (
            <CustomText>
              {' '}
              {textAwal}
              <CustomText style={styles.bold}>{status}</CustomText>
              {textAkhir}
            </CustomText>
          )}
        </CustomText>
  ...
  const IconComponent = statusToIcon[status] || null;

  return (
    <View
      className={`flex-row p-4 items-center shadow-sm bg-white ${
        isViewed === true ? 'bg-sky-100' : ''
      }`}>
      <View className="relative mr-4">
        <Image source={imageSource} className="w-14 h-14 rounded-lg" />
         {IconComponent && (
        <IconComponent className="w-5 h-5 absolute bottom-0 right-0 transform translate-y-0.5 translate-x-0.5" />
      )}
      </View>
      <View className="flex-1">
        ...
        <CustomText className="text-gray-800 mt-1 mr-4 text-xs">
          Your submission
          {submission ? (
            <CustomText style={styles.italic} className="text-xs">
              {' '}
              "{submission}"{' '}
            </CustomText>
          ) : null}
          {total ? (
            <CustomText className="text-xs">
              {' '}
              with a total cost of {total} has been{' '}
              <CustomText style={styles.bold}>{status}</CustomText>. Please
              check your BRIMO application. Thank you.
            </CustomText>
          ) : null}
          {renderStatusText()}
        </CustomText>
